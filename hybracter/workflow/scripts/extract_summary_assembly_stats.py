...
def summarise_contigs(assembly_info, sample, assembly_cleaned_out, summary_out):
    ...
    assembly_df = pd.read_csv(
        assembly_info, delimiter="\t", index_col=False, header=None, names=colnames
    )
    ...
    max_contig_circular = assembly_df[assembly_df["length"] == max_contig].iloc[0][
        "circ"
    ]
