...
def read_gff_csv(gff_csv):
    ...
    gff = pd.read_csv(
        gff_csv, delimiter=",", index_col=False, header=None, names=colnames
    )
    return gff
...
def collate_gffs(is_finder_df_list, cluster_df, count_out_per_sample, counts_matrix):
    ...
    mmseqs = pd.read_csv(
        cluster_df, delimiter="\t", index_col=False, header=None, names=colnames_mmseqs
    )

    merged = pd.merge(total_gff, mmseqs)

    merged["count"] = merged.groupby(["sample", "representative_fasta"])[
        "representative_fasta"
    ].transform("count")
    ...
    count_df = merged[
        ["sample", "representative_fasta", "IS_name", "product", "count"]
    ].drop_duplicates()

    count_df.to_csv(count_out_per_sample, sep=",", index=False)

    count_df_p = merged[["sample", "representative_fasta", "count"]].drop_duplicates()
    count_df_p = count_df_p.pivot(
        index="sample", columns="representative_fasta", values="count"
    ).fillna(0)
